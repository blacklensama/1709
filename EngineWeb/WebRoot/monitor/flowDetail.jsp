...
<link href="../main.css" rel="stylesheet" type="text/css" />
	...
	<script type="text/javascript">
		...
		function main()
		{
			...
			//callUpdateData();
			setTimeout(callUpdateData,200);
		//	setTimeout(callGraphLog,200);
						...
						nodes.length > 0)
					{
						...
						try
						{
							for (var i = 0; i < nodes.length; i++)
							{
								...
								var info = nodes[i].getAttribute('info');
								...
								if (cell != null)
								{
									...
									if (state == 'Running')
									{
										graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#7b68ee', [cell]);
										graph.addCellOverlay(cell, createOverlay(graph.collapsedImage, state +': ' + info));
									}
									else if (state == 'Waiting')
									{
										graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#ff8c00', [cell]);
										graph.addCellOverlay(cell, createOverlay(graph.warningImage, state +': ' + info));
									}
									else if (state == 'Completed')
									{
										graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, '#800000', [cell]);
										graph.addCellOverlay(cell, createOverlay(graph.expandedImage, state +': '+info));
									}
		...
		function callGraphLog(){
			...
			// Get the city and state from the web form
				...
				if ((processId == null) || (processId == "")) return;
				// Build the URL to connect to
				var url = "../MonitorServlet?processId=" + escape(processId)+"&log=true";
				// Open a connection to the server
				xmlHttp.open("GET", url, true);
				// Setup a function for the server to run when it's done
				xmlHttp.onreadystatechange = logReturn;
				// Send the request
				xmlHttp.send(null);
		}
...
<div style="float:right;width:50px;height:30px;padding:0px 0px 0px 0px">
    <input type="text" style='visibility: hidden' id="processId" name="processId" value="<%=processId %>"/>
    <input id="back" type="button"" style='width:30px; height:20px' value="后退"  onclick="javascript:window.history.back();"/>
 </div>  
	<!-- Acts as a container for the graph -->
	<div id="graphContainer" style="overflow:hidden;width:900px;height:1500px;padding:0px 300px 0px 0px;" >
	</div>
	<div id="log" style="width:200px; height:1500px;padding:0px 0px 900px 0px;">
