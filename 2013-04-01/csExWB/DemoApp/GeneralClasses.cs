...
namespace IfacesEnumsStructsClasses
{
    ...
    public class HTMLEditHelper
    {
        ...
        public bool setCharset()
        {
            if (m_pDoc2 != null)//乱码BUG
            {
                string str = "gb2312";
        ...
        public bool AppendDatalink(string targetKeyNameTxt, string targetValueTxt, string linkKeyNameTxt) 
        {          

            string tkname = targetKeyNameTxt;
            string tkval = targetValueTxt;
            string lkval = linkKeyNameTxt; 

            IHTMLElement t = m_pDoc2.createElement("div") as IHTMLElement;
            //t.innerHTML += "datalink\"keyname=\"项目名字\"value=\"项目取值\"target=\"项目名字\"\"";
            string innerHtml = "";
            innerHtml += "已定义数据连接,右击查看连接定义" ;
              ...
              ele.setAttribute("class", "", 0);
              ele.setAttribute("mapKeyName", "textarea",0);
              ele.setAttribute("targerKeyname", tkname, 0);
              ele.setAttribute("targerValue", tkval, 0);
              ele.setAttribute("valueKeyname", lkval, 0);
            ...
            //return (body.insertBefore(nd,(object)body)!=null);
            return (body.appendChild(nd) != null);
        }

        public string getHtmlChanged() {
            //IHTMLDocument2 m_pDoc3 = new IHTMLDocument2();
            string htmlSource = m_pDoc2.body.innerHTML;
            //IHTMLElement body = m_pDoc2.createElement("body") as IHTMLElement;
            IHTMLDOMNode bodyNode = (IHTMLDOMNode)m_pDoc2.body;

            IHTMLDOMChildrenCollection children = (IHTMLDOMChildrenCollection)bodyNode.childNodes;
            foreach (IHTMLDOMNode child in children)
            {
                IHTMLElement childele = (IHTMLElement)child;
                try
                {
                    if (childele.getAttribute("mapKeyName", 0) != null
                        && ((string)childele.getAttribute("mapKeyName", 0)) != string.Empty)
                    {
                        //处理dataMap
                        string targerKeyname = (string)childele.getAttribute("targerKeyname", 0);
                        string targerValue = (string)childele.getAttribute("targerValue", 0);
                        string valueKeyname = (string)childele.getAttribute("valueKeyname", 0);
                        string valueValue = DataBase.getValue(targerKeyname, targerValue, valueKeyname);
                        if (valueValue == null) 
                        {
                            valueValue = "没有找到相应的目标值，请检查链接定义是否正确。"; 
                        }
                        childele.innerHTML = valueValue;
                    }
                }
                catch (Exception e)
                {
                ...
                try
                {
                    if (childele.getAttribute("mapSourceId", 0) != null && ((string)childele.getAttribute("mapType", 0)) != string.Empty)
                    {
                        childele.innerHTML += "处理后的地图图层";//处理地图链接
                    }
                }
                catch (Exception e)
                {
            ...
            string htmlChanged = m_pDoc2.body.parentElement.outerHTML;
            m_pDoc2.body.innerHTML = htmlSource;
            return htmlChanged;
        }
        ...
        { 
            if (m_pDoc2 == null)
                return string.Empty;
            return m_pDoc2.body.parentElement.outerHTML;
        }

        public void setHtmlSource(string source)
        {
            string sourceInner = source.Substring(6, source.Length - 7);
            m_pDoc2.body.parentElement.innerHTML = source;
        }

        //bordersize 2 or "2"
        public bool AppendDiv(string height, string width, int borderSize, string htmlSource, string mapType, string mapSourceId)
        {

            if (m_pDoc2 == null)
                return false;

            IHTMLElement div = m_pDoc2.createElement("div") as IHTMLElement;

            string innerHtml = "已定义地图链接,右击查看连接定义";

            //Image img = Image.FromFile(@"c:\ICON\" + mapType + ".jpg");
            string url = "C:\\ICON\\" + mapType + "X.jpg";
            ...
            div.setAttribute("mapWidth", width, 0);
            div.setAttribute("mapHeight", height, 0);
            div.setAttribute("mapSourceId", mapSourceId, 0);
            div.setAttribute("mapType", mapType, 0);
            div.setAttribute("htmlSource", htmlSource, 0);
            

            div.innerHTML += innerHtml;

            IHTMLDOMNode nd = (IHTMLDOMNode)div;
            IHTMLDOMNode body = (IHTMLDOMNode)m_pDoc2.body;
            return (body.appendChild(nd) != null);
        ...
        /*  if (m_pDoc2 == null)
                ...
                for (int j = 0; j <= 0; j++)
                {
                    ...
                    if (c != null)
                    {
                        ...
                        if (tcell != null)
                        {
                            ...
                            IHTMLElement temp = (IHTMLElement)tcell;
                            temp.innerHTML = "<div><IMG border=0 hspace=0 alt=\"\" align=baseline src=\"C:\\Users\\Public\\Pictures\\Sample Pictures\\Hydrangeas.jpg\"></div>";
                            temp.innerHTML += "<demoTag id=\"ddeo\">hello</demoTag>";
                            temp.setAttribute("style", "height:" + height, 1);
                        }
                    }
                }
            }
            IHTMLElement div = (IHTMLElement)t;

            div.setAttribute("style", "height:" + height,1);
            //div.innerHTML = "innerHTML";
            //div.outerHTML += "outerHTML ";
            //div.outerText += "outerText";
            ...
            //return (body.insertBefore(nd,(object)body)!=null);
            return (body.appendChild(nd) != null);
            //IHTMLDOMNode body = (IHTMLDOMNode)m_pDoc2.activeElement;*/
        ...
        public string AppendTable(int colnum, int rownum, int bordersize, string alignment, int cellpadding, int cellspacing, string widthpercentage, int widthpixel, string backcolor, string bordercolor, string lightbordercolor, string darkbordercolor)
        {
            if (m_pDoc2 == null)
                return null;
            IHTMLTable t = (IHTMLTable)m_pDoc2.createElement("table");
            ...
            mshtml.HTMLTableClass temp = (mshtml.HTMLTableClass)t;
            string html1 = temp.IHTMLElement_outerHTML;
            string test = Regex.Replace(html1, "\\r\\n", "");
        ...
        public void addControl(string html)
        {
            ...
            if ((m_pDoc2 == null) || (m_pDoc2.selection == null))
                return;

            selobj = m_pDoc2.selection as IHTMLSelectionObject;
            if (selobj == null)
                return;
